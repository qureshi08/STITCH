...
import {
    ChevronLeft, Loader2, Lock, Unlock,
    Scissors, Ruler, FileText, ClipboardList,
    Layers, Package, CheckCircle2, History
} from 'lucide-react';
import { cn } from '@/lib/utils';
import { useAuth } from '@/lib/auth';
import { getGarmentDetail, updateGarment, getCollectionDetail } from '@/lib/api';
...
export default function GarmentDetailPage({ params }: { params: Promise<{ id: string, garmentId: string }> }) {
    ...
    if (!garment) return <div className="p-20 text-center font-serif italic">Garment not found.</div>;
        ...
        <div className="space-y-8 pb-20">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <nav className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-ms-gray">
                    ...
                    <Link href={`/admin/collections/${id}`} className="hover:text-ms-black transition-colors max-w-[100px] truncate">{collection?.name}</Link>
                    <ChevronLeft className="w-3 h-3 rotate-180 opacity-30" />
