...
import {
    ChevronLeft, Lock, Unlock, CheckCircle2,
...
export function CollectionDetailClient({ id, initialCollection, initialGarments, user, initialTab = 'overview' }: CollectionDetailClientProps) {
    ...
    const refreshData = async () => {
        // This can be simplified by using router.refresh() in a real production app
        // but for now we'll allow client-side sync if needed or just use this as a trigger
    };
    ...
    return (
        <div className="space-y-0 pb-20">
            ...
            <div className="pt-8 px-0 sm:px-0">
                ...
                {activeTab === 'garments' && <GarmentList collectionId={id} garments={garments} onRefresh={refreshData} canEdit={true} orgId={user?.organization_id || ''} />}
                {activeTab === 'finance' && <CollectionFinanceTab collection={collection} onUpdate={refreshData} />}
