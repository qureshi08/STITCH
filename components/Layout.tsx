...
export function Sidebar({ role }: { role: 'ADMIN' | 'CLIENT' }) {
    ...
    return (
        <aside className="flex flex-col h-screen border-r border-ms-border bg-white sticky top-0 overflow-y-auto w-64 flex-shrink-0">
            {/* Logo */}
            <div className="px-6 py-7 border-b border-ms-border flex items-center gap-3">
                <Link href={role === 'ADMIN' ? '/admin/dashboard' : '/client/dashboard'} className="block">
                    <h1 className="text-2xl font-black tracking-[0.3em] text-ms-black font-serif">STITCH</h1>
            ...
            <div className="p-4 border-t border-ms-border space-y-3">
                ...
                <div className="flex items-center gap-3 px-3 py-3 bg-ms-beige rounded-lg">
                    ...
                    <button onClick={handleLogout} title="Sign out" className="text-ms-gray hover:text-red-500 transition-colors">
...
export function TopBar({ role }: { role: 'ADMIN' | 'CLIENT' }) {
    const { user } = useAuth();

    return (
        <header className="h-14 border-b border-ms-border bg-white sticky top-0 z-10 px-8 flex items-center justify-between flex-shrink-0">
            <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-ms-black">
                    {role === 'ADMIN' ? 'Studio Owner Access' : 'Client Portal'}
                </span>
            </div>
            ...
            <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] font-bold text-ms-black font-serif italic">{user?.name}</span>
                </div>
            </div>
        </header>
    );
